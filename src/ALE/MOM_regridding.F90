...
use coord_hycom,  only : build_hycom1_target_anomaly
...
public regridding_target_anomaly
...
character(len=*), parameter, public :: regriddingInterpSchemeDoc = &
                 ...
                 " PPM_CWK    (3rd-order accurate for uniform grid)\n"//&
...
subroutine initialize_regridding(CS, GV, US, max_depth, param_file, mdl, coord_mode, &
...
!------------------------------------------------------------------------------
!> HYCOM1 interface target anomaly diagnostic field
subroutine regridding_target_anomaly( remapCS, CS, G, GV, US, h, tv, R, Ri, RiA, &
                                      frac_shelf_h)
  ...
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)),  intent(in)    :: h   !< Current 3D grid obtained after
                                                                   !! the last time step [H ~> m or kg m-2]
  type(thermo_var_ptrs),                      intent(in)    :: tv  !< Thermodynamical variables (T, S, ...)
  real, dimension(SZI_(G),SZJ_(G),CS%nk),     intent(out)   :: R   !< HYCOM1 layer potential density
                                                                   !! [R ~> kg m-3]
  real, dimension(SZI_(G),SZJ_(G),CS%nk+1),   intent(out)   :: Ri  !< Interface density [R ~> kg m-3]
  real, dimension(SZI_(G),SZJ_(G),CS%nk+1),   intent(out)   :: RiA !< Interface density anomaly w.r.t. the
                                                                   !! target densities [R ~> kg m-3]
  real, dimension(SZI_(G),SZJ_(G)), optional, intent(in   ) :: frac_shelf_h !< Fractional ice shelf coverage [nomdim]
  ...
  if ((allocated(tv%SpV_avg)) .and. (tv%valid_SpV_halo < 1)) then
    ...
    call MOM_error(FATAL, "Regridding_target_anomaly called in fully non-Boussinesq mode with "//trim(mesg))
  endif
  ...
  select case ( CS%regridding_scheme )
    ...
    case ( REGRIDDING_HYCOM1 )
      call build_grid_HyCOM1_target_anomaly( G, GV, G%US, h, nom_depth_H, tv, remapCS, CS, R, Ri, RiA, &
                              frac_shelf_h, zScale=Z_to_H )
    case default
      call MOM_error(FATAL,'MOM_regridding, regridding_target_anomaly: '//&
                     'only valid for HYCOM1!')

  end select ! type of grid

end subroutine regridding_target_anomaly
...
!> Builds a the interface target anomalies for a simple HyCOM-like grid with the
!! deepest location of potential density interpolated from the column profile.
subroutine build_grid_HyCOM1_target_anomaly( G, GV, US, h, nom_depth_H, tv, remapCS, CS, &
                                             R, Ri, RiA, frac_shelf_h, zScale )
  type(ocean_grid_type),                     intent(in)    :: G  !< Grid structure
  type(verticalGrid_type),                   intent(in)    :: GV !< Ocean vertical grid structure
  type(unit_scale_type),                     intent(in)    :: US !< A dimensional unit scaling type
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(in)    :: h  !< Existing model thickness [H ~> m or kg m-2]
  ...
  if (.not.CS%target_density_set) call MOM_error(FATAL, "build_grid_HyCOM1_target_anomaly : "//&
        "Target densities must be set before build_grid_HyCOM1_target_anomaly is called.")
  ...
  do j = G%jsc-1,G%jec+1 ; do i = G%isc-1,G%iec+1
    if (G%mask2dT(i,j)>0.) then
      ...
      call build_hycom1_target_anomaly(CS%hycom_CS, remapCS, tv%eqn_of_state, GV%ke, nominalDepth, &
           h(i,j,:), tv%T(i,j,:), tv%S(i,j,:), p_col, &
           R(i,j,:), Ri(i,j,:), RiA(i,j,:), h_neglect=h_neglect, h_neglect_edge=h_neglect_edge)
...
end subroutine build_grid_HyCOM1_target_anomaly
